...
class WhoisData:
    ...
    def safe_parse_date(date_item: Union[datetime, List[datetime]]):
        if isinstance(date_item, list):
            return [pendulum.instance(d) if isinstance(d, datetime) else None for d in date_item]
    ...
    def from_dict(whois_data):
        ...
        return WhoisData(
            domain_info=DomainInfo(
                ...
                updated_date=WhoisData.safe_parse_date(whois_data.get('updated_date', [])),
                creation_date=WhoisData.safe_parse_date(whois_data.get('creation_date', [])),
                expiration_date=WhoisData.safe_parse_date(whois_data.get('expiration_date', [])),
